...
const CarBookingModal = (props) => {
  ...
  const handleDateChange = (newValue) => {
    // Get local date string and create Date from it to preserve local timezone
    const localDateStr = newValue?.format?.('YYYY-MM-DD HH:mm:ss');
    setDateValue(localDateStr ? new Date(localDateStr.replace(' ', 'T')) : newValue?.$d);
  };
      ...
      <CustomModal openModal={open} setModalOpen={handleClose}>
        ...
        >
          <Grid container spacing={2} sx={{ flexGrow: 1 }}>
            ...
            <Grid item xs={12} sm={12} md={8} sx={{ minHeight: 0 }}>
              <RentalMap
                location={locations[focusedField]}
                height={
                  isSmall ? "150px" : (tripType === "hourly" || tripType==='day_wise') ? "400px" : "380px"
                }
